...
- **推荐使用分步发布**：先 fill → 用户确认 → 再 click-publish。但在自动化/批量场景下，可使用一步到位发布（`publish`）跳过手动确认。
- **发布前必须通过内容规范检测**（见下方 Step A.2.5），未通过检测的内容不得发布。
- 图文发布时，没有图片不得发布。
...
### Step A.2.5: 内容规范检测（必做，循环直到通过）

发布前必须对标题和正文进行以下合规检测。**任何 P0 或 P1 项命中时，必须自动重写违规部分并重新检测，循环直到全部通过，不得跳过或仅警告。** 详细规则参见 `references/xhs-compliance-rules.md`。

> ⚠️ **核心原则**：内容生成阶段就应主动规避所有引流和诱导互动话术，不要生成后再修。如果仍然检测到违规项，必须自动重写并重新检测，直到零违规才算内容完成。

#### 🔴 P0 红线（命中 → 自动重写 → 重新检测）

1. **诱导互动**
   - 关键词：双击有惊喜、点赞看答案、关注即送、评论区扣1、回复关键词获取、收藏=白嫖、先收藏以防找不到、互赞、互关、互粉、涨粉
   - 模式：「[动作] + [领/送/获取/免费]」的组合句式

2. **免费送福利引导**
   - 关键词：免费送、抽奖、领红包、点赞领取、关注领、评论领、转发领
   - 模式：「关注/点赞/评论/收藏 + 抽奖/送/领」的组合

3. **站外导流**
   - 关键词：微信、VX、vx、卫星、威信、QQ、加我、私聊、后台找我、看主页、+V、淘宝、天猫、拼多多、闲鱼、tb口令
   - 注意：同时检查谐音变体（V❤、wei xin）和符号替代

4. **违禁词 / 绝对化用语**
   - 绝对化：最好、最佳、最便宜、第一、唯一、NO.1、TOP1、顶级、国家级、世界级、独一无二、首选、独家、遥遥领先、金牌、领袖品牌
   - 虚假承诺：100%有效、无效退款、零风险、特效、速效、1天见效、彻底清除、根治、永久、万能、无副作用、纯天然

#### 🟡 P1 风险（命中 → 自动重写 → 重新检测）

1. **话题堆砌**：正文中 `#` 话题标签数量 > 10 个 → 自动精简到 5-8 个
2. **诱导消费词**：抢疯了、万人疯抢、再不抢就没了、全民免单、秒杀、点击领奖、恭喜获奖 → 自动替换为客观描述
3. **权威性词语**：老字号、中国驰名商标、特供、专供、专家推荐、质量免检 → 自动删除
4. **迷信用语**：招财进宝、化解小人、旺财、风水、算命、提升运气 → 自动删除
5. **金融保证词**：保底收益、稳赚不赔、无风险回报、收益率、返本 → 自动删除或改为客观表述
6. **低质内容**：正文 < 50 字且无信息量 → 自动扩充内容

#### 检测循环流程

```
生成内容 → 规范检测 → 有违规？
  ├─ 是 → 自动重写违规部分 → 再次检测（循环，最多 3 轮）
  └─ 否 → ✅ 内容合规，进入下一步
```

- **最多重写 3 轮**：如果 3 轮后仍有违规项，停止流程并向用户报告剩余问题
- **每轮重写后必须完整重新检测**，不能只检查上一轮修改的部分

### Step A.3: 用户确认（建议步骤，非强制）

> 建议步骤：通过 `AskUserQuestion` 展示即将发布的内容（标题、正文、图片/视频），获得用户确认后继续。自动化场景下可跳过此步骤直接进入 Step A.4。
...
- **Bridge 扩展断连**：Chrome 刷新或重启后扩展可能失联。先跑 `check-login` 诊断，若超时提示用户确认扩展已启用并回一句「好了」再重试。参见 `references/publish-patches.md`。
- **「上传图文」Tab 点击失败**：已修复。XHS 页面有多个同名 tab 副本（隐藏 + 可见），原逻辑 `elementFromPoint` 被遮挡时立即返回 `blocked` 导致跳过可见 tab。修复为 `continue` 继续尝试下一个 tab。参见 `references/publish-patches.md`。
- **发布按钮未找到**：已修复。XHS 发布按钮可能不使用 `button.bg-red` 类（UI 更新）。增加了全量 `button` 搜索回退逻辑。参见 `references/publish-patches.md`。
