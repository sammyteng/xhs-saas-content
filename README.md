...
- 撞车松紧可调（默认 ≥3 维 / 看 6 篇）：`profile.py set --clash-dims 2 --window 10`，或环境变量 `XHS_CLASH_DIMS`/`XHS_WINDOW`，或 `diversity.py check --clash-dims/--window` 临时覆盖；`diversity.py config` 看当前生效值。
