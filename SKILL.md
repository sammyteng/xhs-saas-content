...
- [ ] **内容合规检测已通过**：P0（红线）和 P1（风险）均零命中，检测结果记录在 `content.json` 的 `compliance` 字段中。
...
STEP 2a  ★内容合规检测★：按 `styles/content-compliance.md` 规范扫描正文+标题+标签：
          ...
          - **P1（风险拦截）**：命中任一 → 同样必须修改，有被限流或触发审核的高风险。
          - 检测结果写入 content.json 的 `compliance` 字段：
            { "p0_passed": true/false, "p0_issues": [...], "p1_passed": true/false, "p1_issues": [...], "checked_at": "ISO时间" }
...
| 类别 | 说明 |
|------|------|
| 软性引导互动 | 「觉得有用可以收藏一下」「欢迎评论区讨论」等轻度引导，有被限流风险，必须改掉 |
| 模糊功效宣称 | 「效果立竿见影」「用了就回不去了」等夸张但未触及违禁词的表述，必须弱化或删除 |
| 对比贬低竞品 | 「比 XX 好用 100 倍」等未列数据的主观贬低，必须改为客观对比或删除 |
| 敏感话题边缘 | 涉及薪资/裁员/政策评价等可能触发审核的敏感内容，必须规避或模糊化处理 |
