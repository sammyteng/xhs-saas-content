...
STEP 0a  ★读偏好 / 首轮一次配齐★：python3 scripts/profile.py show
          - **首次运行（profile 空）→ 问齐这几项并保存**（之后自动复用）：
             ...
             2. **内容风格**：Agent 按内容类型从 `article-styles.json` 的 8 种（A-H）**自动推荐 1 个并默认用**；用户给编号或关键词（"锐评/教程/吐槽"）可改，**不强制报 ID**。
...
STEP 0b  ★选内容来源（3 选 1，决定本篇选题/素材从哪来）★：
          - **模式 1 · 素材提取（本地资料 或 在线链接）**：用户给参考素材，Agent 读取消化后提炼【候选选题 + 卖点/观点素材】：
            ...
            · ⚠️ **抓来的是参考素材、不照搬**：提炼其中的观点/事实/数据后，按所选风格**重写成原创**小红书，别洗稿（注意 P0-2 虚假体验 + 版权；转述他人观点用"看到一种说法/有人提到"等口径，不冒充亲历）。
...
STEP 3   ★生成封面 + 配图（成品必出图，照 STEP 1 文案做）★：先按正文规划 1-9 张图的 `image_prompts`（第 1 张作封面：大标题+副标题），再**按降级链逐张真出图**：
          ① 有生图能力/key（Agent 自带 或 配了 API key）→ AI 生图：
             - 封面：**先问/判断有没有人物照**——有 → `cover/` 出**人物封面**：**Agent 按内容/品类自动从 `cover/styles/` 挑一个风格**（用户给关键词/调性如"专业/暖/科技感"就映射到最近的，给确切 ID 就用，**不强制报 ID**），跑 `node cover/scripts/generate.mjs --image 人物照 --style <选定风格ID> --title ...`（首次 `cd cover && npm install`；22 风格、最像真人博主，读 `~/.config/xhs-cover/config.json` 的 key）；没有/不提供人物照 → `gen_image.py` 出**纯设计封面**（默认 image-2）。导出 `design-token.json`。（可问不卡：没回应就按"无人物照"走设计封面。）
